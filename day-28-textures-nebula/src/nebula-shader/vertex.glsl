...
void main() {
    ...
    vUv = uv;
    float n = pnoise(position + uTime*0.7, vec3(5.0));
    vNoise = n;
    float displacement = n/10.0;
    vec3 newPos = position + normal * displacement;
    // 2. Calculate the final position of the vertex on the screen
    // projectionMatrix * modelViewMatrix * vec4(position, 1.0)
    // This converts the 3D world position into 2D screen coordinates
